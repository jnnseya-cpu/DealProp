...
export type FeeKey = "deal-packaging" | "deal-success-fee" | "funding-introduction";

/** Who is invoiced. Never the seller — the seller is supply, not revenue. */
export type FeePayer = "buyer" | "lender";
...
export function feeAmount(
  ...
  switch (key) {
    ...
    case "deal-success-fee":
      return applyBps(appraisal.inputs.purchasePrice, assumptions.successFeeBps);
    case "funding-introduction":
      return applyBps(appraisal.funding.seniorDebt, assumptions.fundingIntroBps);
  }
}
...
export function chargeableFees(context: FeeContext): readonly ChargeableFee[] {
  ...
  return FEE_DEFINITIONS.map((definition): ChargeableFee => {
    const amount = feeAmount(definition.key, context.appraisal, assumptions);
